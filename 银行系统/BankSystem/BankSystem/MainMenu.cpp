	...
	: CDialogEx(CMainMenu::IDD, pParent)
{
	...
	m_childposy = 75;
	m_handle.ReadData();
	m_hIcon = AfxGetApp()->LoadIcon(IDI_ICON_APPICO);
...
void CMainMenu::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_STATIC_REGISTER, m_Static_Register);
}
...
BOOL CMainMenu::OnInitDialog()
{
	...
	m_Static_Mainmenu.SetTextWeight(700);
	m_Static_Mainmenu.SetTextFont(_T("黑体"));
	m_Static_Mainmenu.SetTextColor(RGB(255,255,255));
	m_Static_Mainmenu.Init(439, 12, 200, 50, 0);


	CRect temprect(0, 0, 1020, 658);
	CWnd::SetWindowPos(NULL, 100, 100, temprect.Width(), temprect.Height(), SWP_NOZORDER /*| SWP_NOMOVE*/);
...
void CMainMenu::OnPaint()
{
	...
	//m_Static_Mainmenu.SetTextWeight(int(700 * m_x));
	m_Static_Mainmenu.Init(int(439 * m_x), int(12 * m_y), int(200 * m_x), int(50 * m_y), 0);
