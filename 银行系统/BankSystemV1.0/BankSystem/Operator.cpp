...
void COperator::SaveData()
{
	...
	if(outfile1.is_open())
	{
		char title[55] = "用户名\t账号\t  密码\t\t余额\t    身份证号码\t\t手机号码\t地址";
		outfile1 << title;
		list<UserData> datas = m_UserDatas.GetUserDatas();
		if (!datas.empty())
		{
			...
			while (iter != datas.end())
			{
				...
				data = _T("\n") + tmp.username + _T("\t") + tmp.number
					+ _T("\t") + tmp.password + _T("\t") + money
					+ _T("\t") + tmp.id + _T("\t") + tmp.phone
					+ _T("\t") + tmp.address;
				int len = WideCharToMultiByte(CP_ACP, 0, data, -1, NULL, 0, NULL, NULL);
				char *buffer = new char[len + 1];
				WideCharToMultiByte(CP_ACP, 0, data, -1, buffer, len, NULL, NULL);
				outfile1 << buffer;
				delete[] buffer;
				iter++;
			}
